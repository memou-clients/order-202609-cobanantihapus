...
// IMPORTANT FOR MEMOu CONTROLLER:
// Keep these filenames and this declaration format. The Controller reads script.js
// and maps the 15 Deluxe photo slots from this array.
...
function bindConfig() {
  ...
  String(DELUXE_CONFIG.loveLetter || "")
    .split(/\n\s*\n/)
    .filter(Boolean)
    .forEach(paragraph => {
  ...
  const cover = $("#customerCover");
  if (cover) {
    cover.src = `assets/images/${PHOTOS[0]}`;
    cover.alt = ALTS[0];
  }
...
function startBackgroundAudio() {
  if (!DELUXE_CONFIG.music) return;
  if (!state.bgAudio) {
    state.bgAudio = new Audio(DELUXE_CONFIG.music);
    state.bgAudio.loop = true;
    state.bgAudio.volume = 0.30;
  }
...
function renderMemories() {
  ...
  DELUXE_CONFIG.memories.slice(0,14).forEach((caption, i) => {
    ...
    figure.innerHTML = `
      ...
      <img src="assets/images/${PHOTOS[i + 1]}" alt="${ALTS[i + 1]}" loading="lazy" />
...
function openPhoto(index) {
  playSynth("click");
  $("#photoPreview").src = `assets/images/${PHOTOS[index + 1]}`;
  $("#photoPreview").alt = ALTS[index + 1];
